...
subroutine check_arrays(i1,i2,noffset,npartoftype,npartread,nptmass,nsinkproperties,massoftype,&
                        ...
                        got_eosvars,got_taumean,got_dudt,got_ueqi,got_ttherm,got_presi,got_nucleation,got_iorig, &
...
! use cooling_radapprox,only:od_method
 ...
 logical,         intent(in)    :: got_radprop(:),got_iorig,got_apr_level,got_dudt,got_taumean,got_ueqi,got_ttherm,got_presi
...
!
 if (ieos == 24) then
    ...
    elseif (.not. got_presi) then
       if (id==master .and. i1==1) write(*,"(/,1x,a,/)") 'WARNING: presi not in file'
    endif
 endif
